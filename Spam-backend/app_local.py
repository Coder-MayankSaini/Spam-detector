"""
Simple local Spam Detector for testing
"""
import os
import logging
import traceback
...
try:
    ...
    print("pip install Flask flask-cors PyJWT python-dotenv")
    exit(1)
...
def health_check():
    ...
    return jsonify({
        ...
        'message': 'Local Spam Detector API is running',
        'server': 'Local'
    }), 200
...
def login():
    ...
    try:
        ...
        if not all(k in data for k in ['username', 'password']):
            missing_fields = [k for k in ['username', 'password'] if k not in data]
            return jsonify({'error': f'Missing required fields: {missing_fields}'}), 400
...
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 STARTING LOCAL SPAM DETECTOR")
    print("=" * 50)
    ...
    print("   POST /analyze  - Email analysis (requires auth)")
    print("   GET  /history  - Analysis history (requires auth)")
    print("=" * 50)
